...
RESTARTCONTROLLER
    ...
    mavlink.Init(&serials, &mbridge); // ports selected by SerialPort, ChannelsSource
    msp.Init(&serials); // ports selected by SerialPort
    sx_serial.Init(&serials, &mbridge); // ports selected by SerialPort, ChannelsSource
    cli.Init(&serials);
